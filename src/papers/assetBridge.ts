...
async function persistDataUrl(paperId: string, dataUrl: string): Promise<string | null> {
  const match = /^data:([^;,]*);base64,(.*)$/s.exec(dataUrl);
  if (!match) return null;
  const [, mime, base64] = match;
  const saved = await saveNoteImage(paperId, `image.${extFromMime(mime || "image/png")}`, base64);
